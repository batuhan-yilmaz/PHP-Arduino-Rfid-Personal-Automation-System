$(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('.alert_user').html('<p class="alert alert-success">Yeni personel başarıyla eklendi.</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('#dev_sel').val('0');
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-success">Seçilen personel bilgileri güncellendi</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    bootbox.confirm("Kullanıcıyı silmek istiyormusunuz?", function(result) {
      if(result){
        $.ajax({
          url: 'manage_users_conf.php',
          type: 'POST',
          ...
          success: function(response){
            ...
            if (response == 1) {
              ...
              $('#dev_sel').val('0');
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-success">Seçilen kullanıcı silindi!</p>');
            }
            else{
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.select_btn', function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        $('.alert_user').fadeIn(500);
        $('.alert_user').html('<p class="alert alert-success">Kart seçildi!</p>');
