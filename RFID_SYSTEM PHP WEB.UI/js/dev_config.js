$(document).ready(function(){
		...
		$.ajax({
		  ...
		  success: function(response){
		    ...
		    if (response == 1) {			    
			    ...
			    $('.alert_dev').html('<p class="alert alert-success">Yeni bir cihaz başarıyla eklendi</p>');
	...
	$(document).on('click', '.dev_uid_up', function(){
		...
		bootbox.confirm("Aygıt kimliğini güncellemek istiyormusunuz?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			        ...
			        if(response == 1){			    
				    	$('.alert_dev').fadeIn(500);
			        	$('.alert_dev').html('<p class="alert alert-success">Cihaz kimliği güncellendi</p>');
	...
	$(document).on('click', '.dev_del', function(){
		...
		bootbox.confirm("Cihazı kaldırmak istiyormusunuz?", function(result) {
		if(result){
		     // AJAX Request
		     $.ajax({
		       url: 'dev_config.php',
		         ...
		         // Removing row from HTML Table
		          if(response == 1){
		            ...
		            setTimeout(function () {
		              ...
		              bootbox.alert('Cihaz Kaldırılamadı.');
		          }
		       }
		     });
		   }
	...
	$(document).on('click', '.mode_sel', function(){
		...
		bootbox.confirm("Cihaz modunu değiştirmek istiyormusunuz?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			          	...
			          	if(response == 1){
				            ...
				            setTimeout(function () {
				              ...
				              bootbox.alert('Cihaz Seçilemedi.');
		         	 	}
			       	}
			     });
		   	}
